...
- R-13 页数与文本高度接近，但 **PDF 体积小 24.4%**。
- 因此本策略只能作为**开发/试运行通道**，不能据此声明 DOC 格式的验收级兼容。

### R-13 体积差的归因（2026-09-20 补充）

对两份 PDF 做逐页图片清单比对（数量 / 字节 / 像素尺寸 / 编码格式 / 逐图 sha256）：

| 指标 | WPS 产物 | 冻结 PDF | 差值 |
|---|---:|---:|---:|
| 总字节 | 2,275,856 | 3,011,661 | **−735,805（−24.4%）** |
| 页数 | 43 | 43 | 0 |
| 图片数 | 59 | 59 | **0** |
| 图片字节合计 | 2,117,829 | 2,511,739 | −393,910（−15.7%） |
| 非图片字节 | 158,027 | 499,922 | **−341,895（−68.4%）** |
| 逐图 sha256 相同 | **0 张** | — | — |
| 编码格式分布 | `{jpeg: 59}` | `{jpeg: 42, png: 17}` | — |

三个叠加机制：

1. **PNG 被重编码为 JPEG**。冻结版有 17 张 PNG，WPS 全部输出为 JPEG（有损），
   图像字节因此减少 15.7%。第 1 页的**像素尺寸也被改变**（`dims_equal=False`），
   说明部分图片不只是换编码，还缺了无损保真。
2. **全部 59 张图片都被重新编码**——两份 PDF 之间没有任何一张图片字节相同，
   即使像素尺寸相同的页也是如此（同尺寸但不同字节，属有损重编码）。
3. **非图片部分减少 68.4%**。这是更大的单项差异，最可能是字体子集嵌入策略不同。
   它与第 3 节逐页文本的 1–13 字符差异互相解释：字形/字体来源不同。

页面级方向并不一致（例如第 1 页 WPS 更大 1.49×，第 11 页 WPS 更小 0.42×），
所以不能用「WPS 压缩更狠」一句概括。

**对使用的影响**：体积差**不是内容丢失**（图片数量与页数完全一致），
但图像被**有损重编码且部分页像素尺寸改变**，所以 **WPS 产物不得用于版面视觉证据**。
这一条由实证支撑，不再只是原则性判断。
...
- R-13 的 PDF 体积差异已归因（见第 3 节）：PNG→JPEG 有损重编码 + 全部图片重编码 +
  非图片部分减少 68.4%。**未做**视觉等价性判定；WPS 产物不得用于版面视觉证据。
